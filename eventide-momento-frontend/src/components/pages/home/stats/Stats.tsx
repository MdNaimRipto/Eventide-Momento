...
const Stats = () => {
  return (
    <div className="container grid grid-cols-4 px-0 my-16 h-[500px] border border-secondary1">
      {statsData.map((stat, index) => {
        ...
        const border = index !== 3 ? "border-r border-secondary1" : ""; // last one no border
        ...
        return (
          ...
          >
            <CountUp
              ...
              duration={1}
              separator=","
              className={`${LocalFonts.anton.className} text-7xl font-bold tracking-wider`}
            />

            <span className="mt-3 text-xl tracking-wide">{stat.label}</span>
          </div>
        );
      })}
    </div>
